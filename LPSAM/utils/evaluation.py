...
import SAM.utils.metrics as metrics
from hausdorff import hausdorff_distance
from SAM.utils.visualization import visual_segmentation, visual_segmentation_sets_with_pt
...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        pt = get_click_prompt(datapack, opt)
        # pts = model.auto_prompt_generator.get_pts(imgs)
        # pt= (torch.tensor(pts, device=pt[0][0].device), pt[1])
        # pt = None
        with torch.no_grad():
            start_time = time.time()
            pred = model(imgs, pt)

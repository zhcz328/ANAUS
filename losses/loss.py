...
class InfoNCECriterion(nn.Module):
    ...
    def forward(self, target, pred, tind, pind):
        ...
        loss = loss_a + loss_b
        # new
        # Add region contrastive loss and local consistency loss for all combinations
        # region_loss_aa = self.region_contrastive_loss(logits_aa, same_obj_aa, 1 - same_obj_aa)
        # region_loss_ab = self.region_contrastive_loss(logits_ab, same_obj_ab, 1 - same_obj_ab)
        # region_loss_ba = self.region_contrastive_loss(logits_ba, same_obj_ba, 1 - same_obj_ba)
        # region_loss_bb = self.region_contrastive_loss(logits_bb, same_obj_bb, 1 - same_obj_bb)
        #
        # local_consistency_loss_a = self.local_consistency_loss(pred1, same_obj_aa)
        # local_consistency_loss_b = self.local_consistency_loss(pred2, same_obj_bb)
        #
        # total_region_loss = (region_loss_aa + region_loss_ab + region_loss_ba + region_loss_bb) / 4
        # total_local_consistency_loss = (local_consistency_loss_a + local_consistency_loss_b) / 2
        #
        # loss = loss + total_region_loss + total_local_consistency_loss

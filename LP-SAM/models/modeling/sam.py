...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        self.auto_prompter = auto_prompter
    ...
    def forward(
        ...
        if len(pt[0].shape) == 3:
            ...
            if self.auto_prompter:
                result = self.auto_prompter(imgs, imge, self.prompt_encoder.get_dense_pe())
                # de = result[0].permute(0, 2, 1).view(imgs.size()[0], 256, 32, 32)
                se = result[1]
